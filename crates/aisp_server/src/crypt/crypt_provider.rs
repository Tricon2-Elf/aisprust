...
impl<T: CryptProvider> CryptStream<T> {
    pub fn new_server(encrypt_provider: T, decrypt_provider: T) -> Self {
        ...
        Self {
            ...
            encrypt_ctx: encrypt_provider,
            decrypt_ctx: decrypt_provider,
        }
    }
...
impl<T: CryptProvider> Crypter for CryptStream<T> {
    fn handle_incoming(
        &mut self,
        ...
        match self.state {
            State::ServerWaitingKeyExchange => {
                ...
                self.encrypt_ctx.set_key(&self.encrypt_key);
                self.decrypt_ctx.set_key(&self.decrypt_key);
